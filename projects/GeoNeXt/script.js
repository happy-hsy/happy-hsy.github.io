...
const meshScenes=[['case1','jpg'],['case2','jpg'],['case3','png'],['case4','png'],['case5','jpg'],['case6','jpg']];
...
function initMeshShowcase(){
  const base='assets/mesh_assets/comparison',meshVersion='20260831-reindex',cache=new Map();
  ...
  const sceneBar=document.querySelector('#mesh-scenes');sceneBar.innerHTML='';meshScenes.forEach(([key,ext],index)=>{const caseName=`Case ${index+1}`,button=document.createElement('button');button.className='mesh-scene';button.type='button';button.title=caseName;button.setAttribute('aria-label',caseName);button.innerHTML=`<img src="assets/mesh_assets/input/${key}.${ext}" alt="${caseName}">`;button.addEventListener('click',()=>{state.scene=index;loadBoth(true)});sceneBar.appendChild(button)});
